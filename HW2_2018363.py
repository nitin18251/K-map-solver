...
def min2var(min_b,numVar=4):#it takes min term in binary reduced(eg 1_01) form an return its variable(eg wy`z)
	c=-1
	min_v=''
	variables='ABCD'
	if numVar==3:
...
def find_list(list,element):#for searching in list. it returns thee first index on which it occurs.
	c=-1
	for i in list:
		c+=1
		if i==element:
...
def EPI_find(chart_original,normal):#for finding the EPI for our function
	...
	for i in dic:						#searching for those minterms which has single '*' in the whole column
		ind=find_list(dic[i],"*")			#search for the positon of required minterm
...
def minFunc(sIn, numVar=4):
	...
	for i in All_min:# for checking of wrong minterms
		if i>(2**numVar)-1:
			'''print("!?"*25)
			print("WRONG MINTERM ENTERED -> %s <- FOR %s VARIABLES "%(i,numVar))
			print("!?"*25)
			print("RE RUN THE PROGRAM :[")'''
			return None
	
	if numVar==4 and len(All_min)==16:
		print(1)
		return 1
